...
public class BattleControllerView {
	...
	attackButton4, Object1, Object2;
	@FXML
	private ImageView pokemon, pokemon2;
	@FXML
	private ProgressBar playerHealthBar, enemyHealthBar;
	@FXML
	private Text PokemonName, PokemonName2, statusText, HpPokemon, HpPokemon2;
	@FXML
	private StackPane PokemonInTeam1, PokemonInTeam2, PokemonInTeam3, 
	PokemonInTeam4, PokemonInTeam5, PokemonInTeam6, test;
	@FXML
	private Label roundLabel;
	@FXML
	private VBox roundsContainer;
	@FXML
	...
	private Attackable selectedAttack1, selectedAttack2, 
	selectedAttack3, selectedAttack4;
	...
	public void setSelectedAttacks(List<Attackable> selectedAttacks) {
		selectedAttack1 = selectedAttacks.size() > 0 ? 
				selectedAttacks.get(0) : null;
		selectedAttack2 = selectedAttacks.size() > 1 ? 
				selectedAttacks.get(1) : null;
		selectedAttack3 = selectedAttacks.size() > 2 ? 
				selectedAttacks.get(2) : null;
		selectedAttack4 = selectedAttacks.size() > 3 ? 
				selectedAttacks.get(3) : null;
		attackButton1.setText(selectedAttack1 != null ? 
				selectedAttack1.getName() : "None");
		attackButton2.setText(selectedAttack2 != null ? 
				selectedAttack2.getName() : "None");
		attackButton3.setText(selectedAttack3 != null ? 
				selectedAttack3.getName() : "None");
		attackButton4.setText(selectedAttack4 != null ? 
				selectedAttack4.getName() : "None");
	}

	public void setPokemonAttacks(HashMap<Pokemon, 
			List<Attackable>> pokemonAttacks) {
    ...
    HashMap<Integer, StackPane> teamPositions = initializeTeamPositions();
    displayPokemonsOnBattle(teamPositions);
    determineFirstTurn();
}

private HashMap<Integer, StackPane> initializeTeamPositions() {
...
private void displayPokemonsOnBattle(HashMap<Integer,StackPane> teamPositions) {
	...
	private void switchPokemon(Pokemon newPokemon) {
		if (newPokemon != playerPokemon) {
			...
			pokemon.setImage(playerPokemon.getImageReverse());
			updateHealthBar(playerHealthBar, playerPokemon, HpPokemon);
			setSelectedAttacks(pokemonAttacksMap.getOrDefault(playerPokemon, 
					new ArrayList<>()));
			nextTurn();
		}
	}
	...
	private void configureAttackButtons() {
		...
		attackButton3.setOnAction(event -> {
			if (playerTurn)
				executeAttack(selectedAttack3);
		});
		attackButton4.setOnAction(event -> {
			if (playerTurn)
				executeAttack(selectedAttack4);
		});
	}
	...
	private void displayAttackInfo(Pokemon attacker, Attackable attack, 
			Pokemon defender, double damage) {
		Label attackInfo = new Label(attacker.getName() + " utilise " 
			+ attack.getName() + "  et inflige "
				+ String.format("%.2f", damage) + " dégâts à " 
			+ defender.getName() + ".");
		roundsContainer.getChildren().add(attackInfo);
	}

	private void updateHealthBar(ProgressBar healthBar,
			Pokemon pokemon, Text hpText) {
		double progress = pokemon.getMaxHp() > 0 ? 
				(double) pokemon.getHp() / pokemon.getMaxHp() : 0;
	...
	private void addToRoundHistory(Pokemon attacker, 
			Attackable attack, Pokemon defender, double damage) {
		String roundInfo = attacker.getName() + " utilise " + attack.getName() 
		    + "  et inflige "
				+ String.format("%.2f", damage) + " dégâts à " + defender.getName() 
				+ ".";
	...
	private void addToRoundHistory(Pokemon user, String itemName, 
			Pokemon target, double healing) {
		String roundInfo = user.getName() + " utilise " + itemName 
				+ " et récupère " + String.format("%.0f", healing)
				+ " PV.";
